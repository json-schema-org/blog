...
The top physical locations showed one surprise, with Peru taking the top spot, over the UK and Russia. Otherwise, the USA takes top spot with 5.73 million of those 7.21 million requests.
...
(For reasons I won't go into, we have a meta-schema for Hyper Schema in 2019-09, but not 2020-12.) 1.53 million to `.../meta/core`, and next palace, the validation vocabulary meta-schema, with only 577.84k. If this is accurate, it's a little concerning. We would expect `/draft/2020-12/schema` to be the top path accessed, but it doesn't appear till position 8, with 472.46k.
...
Access of meta-schemas is unlikely to corrolate with useage percentages, but it is at least an indicator of use to an extent.
